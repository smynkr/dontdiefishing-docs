...
export const sourceRoots = ["dontdiefishing"];
...
export function check({ root = process.cwd() } = {}) {
  ...
  // Next rewrites `/` to `/dontdiefishing` (the DontDieFishing index) without
  // replacing fragments, so root links must validate against that canonical
  // document's headings.
  const rootIndex = routeFiles.get("/getting-started") ?? routeFiles.get("/dontdiefishing");
